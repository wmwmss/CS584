# this is the python implementation of recommender systems
...
X_train = X_train.merge(movie_directors, on = 'movieID', how='left')
#X_train.head

Y_train = train.copy()[['rating']]

test = test.copy()
test = test.merge(movie_directors, on = 'movieID', how='left')
#test.head
# pre-preprocessing
# encoding director
lb_make = LabelEncoder()
X_train.groupby('directorID')['directorID'].nunique()
X_train["directorID"].isna().sum()
#X_train[X_train["directorID"].isna()]["directorID"]
X_train["directorID"].fillna("No Director", inplace = True)
X_train["directorID"].isna().sum()

X_train["directorID"].astype('string')
#X_train["directorID"]

X_train["directorIDCat"] = lb_make.fit_transform(X_train["directorID"])
X_train[["directorID","directorIDCat"]].dtypes
#X_train.groupby('directorIDCat')['directorIDCat'].nunique()
#X_train['directorIDCat'].head
del X_train['directorID']
#X_train.head


lb_make1 = LabelEncoder()
#test.groupby('directorID')['directorID'].nunique()
test["directorID"].isna().sum()
#test[test["directorID"].isna()]["directorID"]
test["directorID"].fillna("No Director", inplace = True)
test["directorID"].isna().sum()

test["directorID"].astype('string')
#test["directorID"]

test["directorIDCat"] = lb_make1.fit_transform(test["directorID"])
#test[["directorID","directorIDCat"]].dtypes
#test.groupby('directorIDCat')['directorIDCat'].nunique()
#test['directorIDCat'].head
del test['directorID']
#test.head
...
train_movieIDs
#train.columns[9000][1]

#userIDs = []
train_userIDs = []
train.shape[0]
for c in range(train.shape[0]):
    train_userIDs.append(train.index[c])
train_userIDs
train.index[0]
...
# decision Trees for cold-starter
# 1.27
...
#df_train_movieIDs
#df_train_userIDs
#test.iloc[0]['userID'] in df_train_userIDs.values
test.loc[0]['movieID']
clf.predict([test.iloc[0]])
test.iloc[0]['userID']
corr[75]
userID = test.iloc[0]['userID']
# find userID in train
...
for i in range(count):
    ...
    if userID not in df_train_userIDs.values:
        score = 3.0
        prediction.append(score)
        print("user not in train: count %d, user %d" % (counter, userID))
        counter = counter + 1
        continue

    neighbors = corr[train.index == userID]
    #neighbors
    ...
    neighbors = train.iloc[neighbors_index,:]
    # look for movie in train
    movie = test.loc[i]['movieID']
    if movie not in df_train_movieIDs.values:
        score = lab_enc.inverse_transform(clf.predict([test.iloc[i]]))
        prediction.append(score)
        print("movie not in train: count %d, movie %d. clf predict: %d" % (counter, movie, score))
        counter = counter + 1
        continue
    else:
        try:
        ...
        if non_zero_score_array.size == 0:
            ...
            score = array([np.mean(non_zero_score_array)])
    prediction.append(score)
#pd.DataFrame(prediction).fillna(0, inplace = True)
print("--- %s seconds ---" % (time.time() - start_time))
prediction
prediction = np.concatenate(prediction).ravel()
